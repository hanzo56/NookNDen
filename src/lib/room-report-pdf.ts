...
export async function buildRoomInventoryPdf(params: {
  room: ReportRoom;
  ...
  draw("Room inventory report", { bold: true, size: 18 });
  y -= 6;
  draw(`Room: ${params.room.name}`);
  draw(`Room type: ${params.room.room_type}`);
  draw(`Generated: ${generatedLabel}`);
  draw(`Items in room: ${params.items.length}`);
  y -= 8;
  draw(
    "Fields include sale price and estimated ACV (straight-line, 10% salvage) as of the generation date above.",
    { size: 9 },
  );
  y -= 4;
  ...
  for (const item of params.items) {
    ...
    const rows: [string, string][] = [
      ...
      ["Room location", item.location || params.room.name],
  ...
  if (params.items.length === 0) {
    page = pdfDoc.addPage([LETTER_W, LETTER_H]);
    y = LETTER_H - M;
    draw("No inventory items are assigned to this room.", { size: 12 });
  }
